...
<head>
    ...
    @php
        ...
        // Default canonical - can be overridden by @yield('canonical')
        ...
        $siteName = $siteSettings->site_name ?? 'گروهباز';
        $siteNameEn = $siteSettings->site_name_en ?? 'GroohBaz';
        
        // Get absolute logo URL - CRITICAL for Google recognition
        $logoUrl = null;
        $logoWidth = 512;
        $logoHeight = 512;
        
        if ($siteSettings->logo) {
            // Use url() to ensure absolute URL (required by Google)
            $logoPath = $siteSettings->logo;
            $logoUrl = url($logoPath);
            
            // Try to get actual image dimensions if file exists
            $logoFilePath = public_path($logoPath);
            if (file_exists($logoFilePath) && function_exists('getimagesize')) {
                $imageInfo = @getimagesize($logoFilePath);
                if ($imageInfo !== false) {
            ...
            // Ensure minimum size (Google requires at least 112x112)
            if ($logoWidth < 112) $logoWidth = 512;
            if ($logoHeight < 112) $logoHeight = 512;
        }
        
        $pageImage = $logoUrl ?: url('favicon.ico');
        
        // Social media links for sameAs property
        $socialLinks = [];
        // Telegram link from footer
        $socialLinks[] = 'https://t.me/groohbaz_ir';
        
        // Enhanced Organization Schema for AEO - SINGLE CANONICAL SOURCE
        $organizationSchema = [
            '@context' => 'https://schema.org',
            '@type' => 'Organization',
            'name' => $siteName,
            'alternateName' => $siteNameEn,
            'description' => 'بازار آنلاین خرید و فروش گروه‌های تلگرام، کانال‌های تلگرام، پیج‌های اینستاگرام، سایت‌های آماده، دامنه‌ها و کانال‌های یوتیوب در ایران',
            'url' => url('/'),
            'inLanguage' => 'fa-IR',
            'areaServed' => [
                '@type' => 'Country',
            ...
            'knowsAbout' => [
                'خرید و فروش گروه تلگرام',
                'خرید و فروش کانال تلگرام',
                'خرید و فروش پیج اینستاگرام',
                'خرید و فروش سایت آماده',
                'خرید و فروش دامنه',
                'خرید و فروش کانال یوتیوب'
            ],
            'serviceType' => 'Marketplace',
            'audience' => [
                '@type' => 'Audience',
                'audienceType' => 'خریداران و فروشندگان دارایی‌های دیجیتال'
            ]
        ];
        
        // Add logo with absolute URL (REQUIRED by Google)
        if ($logoUrl) {
            $organizationSchema['logo'] = [
                '@type' => 'ImageObject',
                'url' => $logoUrl, // Absolute URL required
                'width' => $logoWidth,
                'height' => $logoHeight
            ];
        }
        
        // Add social media links (sameAs property)
        if (!empty($socialLinks)) {
            $organizationSchema['sameAs'] = $socialLinks;
        }
        
        // WebSite Schema - references Organization as publisher
        $jsonLdData = [
            '@context' => 'https://schema.org',
            '@type' => 'WebSite',
            'name' => $siteName,
        ...
        // Add logo to WebSite schema as well (Google can read from either)
        if ($logoUrl) {
            $jsonLdData['logo'] = [
                '@type' => 'ImageObject',
                'url' => $logoUrl, // Absolute URL required
    ...
    @endphp
    <!-- Favicon & App Icons for SEO - Using absolute URLs for consistency -->
    @if($logoUrl)
    ...
    @else
        ...
        @endphp
        <link rel="icon" type="image/x-icon" sizes="32x32" href="{{ $faviconUrl }}">
    ...
    <meta name="author" content="{{ $siteSettings->site_name ?? 'GroohBaz' }}">
    ...
    <meta property="og:image:alt" content="{{ $siteSettings->site_name ?? 'GroohBaz' }}">
    <meta property="og:site_name" content="{{ $siteSettings->site_name ?? 'GroohBaz' }}">
    ...
    <meta name="twitter:image:alt" content="{{ $siteSettings->site_name ?? 'GroohBaz' }}">
    ...
    <style>
        ...
        .navbar-modern{background:rgba(17,17,24,0.85)!important;backdrop-filter:blur(10px);border-bottom:1px solid rgba(255,255,255,0.1);transform:translateZ(0);will-change:transform}
        .glass-card{background:rgba(255,255,255,0.05);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,0.1);border-radius:20px;will-change:transform}
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-modern fixed-top">
        <div class="container">
            <a class="navbar-brand navbar-brand-modern d-flex align-items-center gap-2" href="{{ route('home') }}">
                @if($logoUrl)
                    <img src="{{ $logoUrl }}" alt="{{ $siteSettings->site_name }}" width="40" height="40" style="height: 40px; width: auto;">
                @endif
            ...
            <div class="row">
                <div class="col-md-4 mb-3 mb-md-0">
                    <div class="mb-3 d-flex align-items-center gap-2">
                        @if($logoUrl)
                            <img src="{{ $logoUrl }}" alt="{{ $siteSettings->site_name }}" width="40" height="40" loading="lazy" style="height: 40px; width: auto;">
                        @endif
    ...
    <script defer src="{{ asset('assets/js/performance-optimizations.js') }}"></script>
    <!-- Custom JavaScript -->
