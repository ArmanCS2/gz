...
<head>
    ...
    <title>@yield('title', 'پنل مدیریت - GroohBaz')</title>
    ...
    <style>
        * {
        ...
        .sidebar.collapsed #adminSidebarMenu h6 {
            display: none;
        }
        ...
        .sidebar.collapsed ~ #adminMainContent {
            margin-right: 0;
            width: calc(100% - 80px);
        }
        ...
        /* Tables in admin - Force Dark Theme */
        .table,
        table.table {
            color: #ffffff !important;
        ...
        /* Bootstrap modal z-index is 1055, backdrop is 1050, so datepicker must be much higher */
        .jdp-container,
        .jdp-wrapper,
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark">
        <div class="container-fluid">
            <button class="btn btn-modern btn-sm d-md-none me-2" type="button" data-bs-toggle="offcanvas" data-bs-target="#adminSidebarOffcanvas" aria-controls="adminSidebarOffcanvas" style="background: rgba(255,255,255,0.1); box-shadow: none;">
                <i class="bi bi-list"></i>
            </button>
            <a class="navbar-brand fw-bold" href="{{ route('admin.dashboard') }}" style="background: linear-gradient(135deg, #00f0ff, #b026ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">پنل مدیریت</a>
    ...
    <div class="container-fluid">
        <div class="row">
            <!-- Desktop Sidebar -->
            <div class="col-md-3 sidebar p-3 d-none d-md-block" id="adminDesktopSidebar">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h6 class="mb-0 fw-bold" style="color: #ffffff;">منو</h6>
                    <button class="btn btn-sm d-none d-lg-block" type="button" id="adminToggleSidebar" style="background: rgba(255,255,255,0.1); color: #ffffff; border: none; padding: 4px 8px; border-radius: 6px;">
                        <i class="bi bi-chevron-right" id="adminSidebarIcon"></i>
                    </button>
                </div>
                <ul class="nav flex-column" id="adminSidebarMenu">
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item mb-2">
                        <a class="nav-link text-white {{ request()->routeIs('admin.settings') ? 'active bg-primary' : '' }}" href="{{ route('admin.settings') }}">
                            <i class="bi bi-gear me-2"></i> <span>تنظیمات سایت</span>
                        </a>
                    </li>
                    <li class="nav-item mb-2">
                        <a class="nav-link text-white {{ request()->routeIs('admin.categories.*') ? 'active bg-primary' : '' }}" href="{{ route('admin.categories.index') }}">
                            <i class="bi bi-grid-3x3-gap me-2"></i> <span>دسته‌بندی‌ها</span>
                        </a>
                    </li>
                    <li class="nav-item mb-2">
                        <a class="nav-link text-white {{ request()->routeIs('admin.rules.*') ? 'active bg-primary' : '' }}" href="{{ route('admin.rules.index') }}">
                            <i class="bi bi-file-text me-2"></i> <span>قوانین سایت</span>
                        </a>
                    </li>
                    <li class="nav-item mb-2">
                    ...
                    <li class="nav-item mb-2">
                        <a class="nav-link text-white {{ request()->routeIs('admin.ads.*') ? 'active bg-primary' : '' }}" href="{{ route('admin.ads.index') }}">
                            <i class="bi bi-card-list me-2"></i> <span>آگهی‌ها</span>
                        </a>
                    </li>
                    <li class="nav-item mb-2">
                        <a class="nav-link text-white {{ request()->routeIs('admin.reviews.*') ? 'active bg-primary' : '' }}" href="{{ route('admin.reviews.index') }}">
                            <i class="bi bi-chat-left-text me-2"></i> <span>نظرات و امتیازها</span>
                        </a>
                    </li>
                    <li class="nav-item mb-2">
                        <a class="nav-link text-white {{ request()->routeIs('admin.auctions.*') ? 'active bg-primary' : '' }}" href="{{ route('admin.auctions.index') }}">
                            <i class="bi bi-hammer me-2"></i> <span>مزایده‌ها</span>
                        </a>
                    </li>
                    <li class="nav-item mb-2">
                        <a class="nav-link text-white {{ request()->routeIs('admin.otp.*') ? 'active bg-primary' : '' }}" href="{{ route('admin.otp.index') }}">
                            <i class="bi bi-shield-check me-2"></i> <span>لاگ OTP</span>
                        </a>
                    </li>
                    <li class="nav-item mb-2">
                        <a class="nav-link text-white {{ request()->routeIs('admin.blog.*') ? 'active bg-primary' : '' }}" href="{{ route('admin.blog.posts.index') }}">
                            <i class="bi bi-journal-text me-2"></i> <span>مدیریت بلاگ</span>
                        </a>
                    </li>
                </ul>
            </div>
            ...
            <div class="offcanvas offcanvas-start" tabindex="-1" id="adminSidebarOffcanvas" aria-labelledby="adminSidebarOffcanvasLabel" style="background: var(--glass-bg); backdrop-filter: blur(20px) saturate(180%); -webkit-backdrop-filter: blur(20px) saturate(180%); border-right: 1px solid var(--glass-border);">
                <div class="offcanvas-header border-bottom" style="border-color: rgba(255,255,255,0.1);">
                    <h5 class="offcanvas-title fw-bold" id="adminSidebarOffcanvasLabel" style="color: #ffffff;">منو</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                </div>
                <div class="offcanvas-body p-3">
                    <ul class="nav flex-column">
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('admin.dashboard') ? 'active bg-primary text-white' : '' }}" href="{{ route('admin.dashboard') }}" data-bs-dismiss="offcanvas">
                                <i class="bi bi-speedometer2"></i> داشبورد
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('admin.settings') ? 'active bg-primary text-white' : '' }}" href="{{ route('admin.settings') }}" data-bs-dismiss="offcanvas">
                                <i class="bi bi-gear"></i> تنظیمات سایت
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('admin.categories.*') ? 'active bg-primary text-white' : '' }}" href="{{ route('admin.categories.index') }}" data-bs-dismiss="offcanvas">
                                <i class="bi bi-grid-3x3-gap"></i> دسته‌بندی‌ها
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('admin.rules.*') ? 'active bg-primary text-white' : '' }}" href="{{ route('admin.rules.index') }}" data-bs-dismiss="offcanvas">
                                <i class="bi bi-file-text"></i> قوانین سایت
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('admin.users.*') ? 'active bg-primary text-white' : '' }}" href="{{ route('admin.users.index') }}" data-bs-dismiss="offcanvas">
                                <i class="bi bi-people"></i> کاربران
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('admin.ads.*') ? 'active bg-primary text-white' : '' }}" href="{{ route('admin.ads.index') }}" data-bs-dismiss="offcanvas">
                                <i class="bi bi-card-list"></i> آگهی‌ها
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('admin.reviews.*') ? 'active bg-primary text-white' : '' }}" href="{{ route('admin.reviews.index') }}" data-bs-dismiss="offcanvas">
                                <i class="bi bi-chat-left-text"></i> نظرات و امتیازها
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('admin.auctions.*') ? 'active bg-primary text-white' : '' }}" href="{{ route('admin.auctions.index') }}" data-bs-dismiss="offcanvas">
                                <i class="bi bi-hammer"></i> مزایده‌ها
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('admin.otp.*') ? 'active bg-primary text-white' : '' }}" href="{{ route('admin.otp.index') }}" data-bs-dismiss="offcanvas">
                                <i class="bi bi-shield-check"></i> لاگ OTP
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('admin.blog.*') ? 'active bg-primary text-white' : '' }}" href="{{ route('admin.blog.posts.index') }}" data-bs-dismiss="offcanvas">
                                <i class="bi bi-journal-text"></i> مدیریت بلاگ
                            </a>
                        </li>
                    </ul>
                </div>
            ...
            <div class="col-md-9 col-lg-9 p-4" id="adminMainContent">
                {{ $slot }}
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const sidebarIcon = document.getElementById('adminSidebarIcon');
