...
@push('meta')
    ...
    <meta property="og:image" content="{{ url($post->banner_image) }}">
    @endif
    ...
    @if($post->banner_image)
    <meta name="twitter:image" content="{{ url($post->banner_image) }}">
    @endif
    ...
    @php
        ...
        // Get absolute logo URL - must match main Organization schema
        $logoUrl = null;
        $logoWidth = 512;
        $logoHeight = 512;
    ...
    <script type="application/ld+json">
    {
        ...
        "image": @if($post->banner_image) "{{ url($post->banner_image) }}" @else null @endif,
        ...
        "publisher": {
            ...
            "name": "{{ $siteName }}",
            @if($publisherLogo)
            "logo": {!! json_encode($publisherLogo, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) !!}
            @endif
        }
    }
    </script>
