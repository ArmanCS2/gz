...
(function() {
    ...
    // Search filter visual feedback - Debounced to prevent excessive updates
    let filterUpdateTimeout = null;
    
    document.addEventListener('livewire:init', () => {
        if (typeof Livewire !== 'undefined') {
            Livewire.hook('morph.updated', () => {
                // Debounce filter visual updates to prevent excessive DOM queries
                clearTimeout(filterUpdateTimeout);
                filterUpdateTimeout = setTimeout(updateFilterVisuals, 50);
            });
        }
    });
    ...
    function updateFilterVisuals() {
        ...
        const elements = document.querySelectorAll('.modern-input, .modern-select select, .segmented-control button');
    ...
    // Modal Scrollable Helper - Optimized MutationObserver with debouncing
    ...
    // Optimize MutationObserver - only watch for modal additions, not entire body
    const observer = new MutationObserver(function(mutations) {
        // Debounce to prevent excessive calls
        clearTimeout(modalUpdateTimeout);
        modalUpdateTimeout = setTimeout(() => {
            // Only process if modals are actually added
            const hasNewModals = mutations.some(mutation => 
                Array.from(mutation.addedNodes).some(node => 
                    node.nodeType === 1 && (node.classList?.contains('modal') || node.querySelector?.('.modal-dialog'))
                )
            );
            if (hasNewModals) {
                makeModalsScrollable();
    ...
    // Also add to dynamically created modals with Livewire - Optimized
    document.addEventListener('livewire:init', () => {
        makeModalsScrollable();
        if (typeof Livewire !== 'undefined') {
            // Debounce Livewire updates to prevent excessive calls
            let livewireUpdateTimeout = null;
            Livewire.hook('morph.updated', () => {
                clearTimeout(livewireUpdateTimeout);
                livewireUpdateTimeout = setTimeout(makeModalsScrollable, 50);
            });
        }
    });
...
function initBlogFeatures() {
    ...
    // Reading progress bar - Optimized with requestAnimationFrame and passive listeners
    ...
    function updateReadingProgress() {
        // Use requestAnimationFrame for smooth updates without blocking scroll
        if (rafId) return;
        ...
        rafId = requestAnimationFrame(() => {
            ...
            readingProgress.style.transform = `scaleX(${Math.min(progress, 100) / 100})`;
            readingProgress.style.transformOrigin = 'right';
