...
class Index extends Component
{
    ...
    public function render()
    {
        ...
        // SEO Rules: Determine if page should be indexed
        // Base /store page (no filters) = INDEX
        // Any filters active = NOINDEX, FOLLOW
        $hasFilters = !empty($this->search) || 
                     !empty($this->category) || 
                     $this->type !== 'all' ||
                     !empty($this->min_members) ||
                     !empty($this->max_members) ||
                     !empty($this->min_price) ||
                     !empty($this->max_price) ||
                     $this->sort !== 'latest';
        
        $robotsMeta = $hasFilters ? 'noindex, follow' : 'index, follow';
        $canonicalUrl = $hasFilters ? route('store.index') : url()->current();
        ...
        return view('livewire.store.index', [
            ...
            'title' => $hasFilters ? 'جستجوی آگهی‌ها | ' . ($siteSettings->site_name ?? 'گروه باز') : 'فروشگاه | ' . ($siteSettings->site_name ?? 'گروه باز'),
            'description' => $hasFilters ? 'نتایج جستجو و فیلتر آگهی‌ها' : 'بازار خرید و فروش گروه‌های تلگرام، کانال‌های تلگرام و پیج‌های اینستاگرام',
