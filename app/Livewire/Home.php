...
class Home extends Component
{
    ...
    public function render()
    {
        ...
        // SEO Rules: Homepage with filters = NOINDEX, FOLLOW
        // Clean homepage (no filters) = INDEX, FOLLOW
        $hasFilters = !empty($this->searchQuery) || 
                     !empty($this->category) || 
                     $this->listingType !== 'all' ||
                     $this->minMembers > 0 ||
                     !empty($this->maxMembers) ||
                     $this->minPrice > 0 ||
                     $this->maxPrice > 0;
        
        $robotsMeta = $hasFilters ? 'noindex, follow' : 'index, follow';
        $canonicalUrl = $hasFilters ? route('home') : url()->current();
